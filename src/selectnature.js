...
import { Form,Loader,Container,Header,Icon,Button,Modal } from 'semantic-ui-react';
...
class Selectnature extends Component {
  ...
  constructor(props){
    super(props)
    this.state = {loading:true,expertise:[],pictures: [],brands:[],openmodaluserinformations: false}
  ...
  dataTransmissionToTheDealer = async()=>{
    ...
    if(!this.state.useremail){
      ...
      this.setState({openmodaluserinformations:true});
  ...
  inputHandleChange = (event,data)=>{
    ...
    if(inputvalue && !this.focus){
      this.expertise = ([...this.state.expertise,{[labelvalue]:inputvalue}]);
      this.setState({expertise:this.expertise});
    }
    ...
    if(this.state.configurationformlist!==undefined && this.state.nature){
      ...
      const InputFromAddonsOfNatureSelected = addOnsOfNatureSelected.map(
        ...
        addOnsCatalog[element].data?
          <div>
            <Form.Input 
              key={addOnsCatalog[element].property && addOnsCatalog[element].property}
              label={addOnsCatalog[element].title && addOnsCatalog[element].title} 
              placeholder={addOnsCatalog[element].title && addOnsCatalog[element].title}
              list={addOnsCatalog[element].property && addOnsCatalog[element].property}
  ...
  render() {
    ...
    const { openmodaluserinformations } = this.state;
        ...
        <Container className="natureselect">
          <Header as='h3' className='headertitle'>
            ...
            <Header.Content>Dites-nous en plus sur votre machine.</Header.Content>
          ...
          
              {
                ...
                <div>
                  ...
                  {this.state.nature &&<Form.Input id='useremail' iconPosition='left' label='Votre email' placeholder='Email'>
                    <Icon name='at' />
                    <input  ref={this.emailinput} value={this.state.useremail} onChange={(event)=>this.setState({useremail:event.target.value})}/>
                  </Form.Input>
                  }
                  {this.state.nature &&<Form.Input iconPosition='left' label='Votre numéro de téléphone' placeholder='Portable'>
                    <Icon name='phone' />
                    <input ref={this.phoneinput} value={this.state.userphone} onChange={(event)=>this.setState({userphone:event.target.value})}/>
                  </Form.Input>
                  }
                  {
              ...
              <Modal
                open={openmodaluserinformations}
                onOpen={() => this.setState({ openmodaluserinformations: true })}
                onClose={() => this.setState({ openmodaluserinformations: false })}
                size='small'
              >
                <Modal.Header>Presque !</Modal.Header>
                <Modal.Content>
                  <p>Saisissez votre email et votre téléphone pour que nous puissions vous transmettre noptre proposition.</p>
                </Modal.Content>
                <Modal.Actions>
                  <Button icon='check' content="J'ai compris" onClick={() => this.setState({ openmodaluserinformations: false })} />
                </Modal.Actions>
              </Modal>
        </Container>
